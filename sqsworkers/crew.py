...
class Crew():
    ...
    def __init__(self, **kwargs):
        self.crew_name = 'crew-%s-%s-%s' % (os.getpid(), kwargs['queue_name'], str(time.time()))
        self.workers = []
        self.supervisor = None

        self.sqs_session = kwargs['sqs_session']
        self.queue_name = kwargs['queue_name']
        self.MessageProcessor = kwargs['MessageProcessor']
        self.logger = logging.LoggerAdapter(kwargs['logger'], extra={'extra': {'crew_name': self.crew_name}})
...
class Worker(CrewMember):
    def __init__(self, crew):
        ...
        self.logger = logging.LoggerAdapter(self.crew.logger, extra={'extra': {'worker_name': self.worker_name, 'crew_name': self.crew.crew_name}})
...
class Supervisor(CrewMember):
    ...
    def _wrap_run(self):
        try:
            ...
            log_uncaught_exception(e, logger=self.crew.logger, context={'supervisor_name': self.supervisor_name, 'crew_name': self.crew.crew_name})
            self._wrap_run()
    ...
    def supervise(self):
        ...
        for worker in self.crew.workers:
            if worker.is_alive():
                ...
                self.crew.statsd.increment('workers.dead', 1, tags=[self.crew.crew_name])
        ...
        self.crew.statsd.gauge('workers.employed', len(good_workers), tags=[self.crew.crew_name])
